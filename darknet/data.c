...
void fill_truth_swag(char *path, float *truth, int classes, int flip, float dx, float dy, float sx, float sy)
{
	...
	correct_boxes(boxes, count, dx, dy, sx, sy, flip, 0, 0);
	float x, y, w, h;
	int id;
	int i;
...
{
	char labelpath[4096];
	find_replace(path, "yolotraindest", "yolotrainlabel", labelpath);
	//find_replace(labelpath, "JPEGImages", "labels", labelpath);
...
void fill_truth_detection(char *path, int num_boxes, float *truth, int classes, int flip, float dx, float dy, float sx, float sy, int vflip, int trans)
{
	char labelpath[4096];
	find_replace(path, "yolotraindest", "yolotrainlabel", labelpath);
	//find_replace(labelpath, "JPEGImages", "labels", labelpath);
	...
	box_label *boxes = read_boxes(labelpath, &count);
	randomize_boxes(boxes, count);
	correct_boxes(boxes, count, dx, dy, sx, sy, flip, vflip, trans);
	if (count > num_boxes) count = num_boxes;
	float x, y, w, h;
	int id;
	int i;
...
void fill_truth_autoparamet(char *path, int datalength, float *truth)
{
	char labelpath[4096];
	find_replace(path, "yolotraindest", "yolotrainlabel", labelpath);
	//find_replace(labelpath, "JPEGImages", "labels", labelpath);
	...
	fscanf(file, "%f %f %f %f %f ", &truth[0], &truth[1], &truth[2], &truth[3], &truth[4]);
	fscanf(file, "%f %f %f %f %f ", &truth[5], &truth[6], &truth[7], &truth[8], &truth[9]);
	fscanf(file, "%f %f %f %f %f ", &truth[10], &truth[11], &truth[12], &truth[13], &truth[14]);
	fscanf(file, "%f %f %f %f %f ", &truth[15], &truth[16], &truth[17], &truth[18], &truth[19]);
	fclose(file);
}
...
data load_data_autoparamet(int n, char **paths, int m, int w, int h, int boxes, int classes,
	float jitter, float hue, float saturation, float exposure)
{
	char **random_paths = get_random_paths(paths, n, m);
	int i;
	...
	for (i = 0; i < n; ++i) {
		...
		fill_truth_autoparamet(random_paths[i], 20, d.y.vals[i]);
		//fill_truth_detection(random_paths[i], boxes, d.y.vals[i], classes, flip,
		//	-dx / w, -dy / h, nw / w, nh / h, vflip, trans);
