...
//yolov2检测层 bias_match=1/classes = 80/coords = 4/n = 5/softmax = 1/jitter = .3/rescore = 1
...
void forward_region_layer(const layer l, network net)
{
    ...
    *(l.cost) = 0;
    ...
    for (b = 0; b < l.batch; ++b) {
        ...
        for (j = 0; j < l.h; ++j) {
            for (i = 0; i < l.w; ++i) {
				//n是num,在这表示5,不同输出通道425
                for (n = 0; n < l.n; ++n) { 	
					//每个n下有(classes + coords + 1)=85层,先放四个box
                    int box_index = entry_index(l, b, n*l.w*l.h + j*l.w + i, 0);
                    box pred = get_region_box(l.output, l.biases, n, box_index, i, j, l.w, l.h, l.w*l.h);
                    float best_iou = 0;
					//可知标签truth数据,这里有些问题，和yolov1不一样，每个格子有不同数据
					//感觉是每张图对应30个box
                    for(t = 0; t < 30; ++t){
                        box truth = float_to_box(net.truth + t*(l.coords + 1) + b*l.truths, 1);
                        if(!truth.x) break;
					...
					//每个n下有(classes + coords + 1)=85层,四个box之后放classes80物品概率
...
void get_region_detections(layer l, int w, int h, int netw, int neth, float thresh, int *map, float tree_thresh, int relative, detection *dets)
{
    ...
    for (i = 0; i < l.w*l.h; ++i){
        ...
        for(n = 0; n < l.n; ++n){
			...
			//每个n下有(classes + coords + 1)=85层,再存放概率
