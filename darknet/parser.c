...
//这个参数里的h,w,c是表示网络当前层的输入特征图的长/宽/个数，分不断变化
...
void load_convolutional_weights_binary(layer l, FILE *fp)
{
	...
	int size = l.c*l.size*l.size;
	int i, j, k;
	//从这可以看到，卷积层参数是以 输出/输入/size 排列，以二值化方式存的，故读的方式和正常不同
	for (i = 0; i < l.n; ++i) {
		float mean = 0;
