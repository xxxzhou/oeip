...
int32_t FLiveInput::open(const char* url) {
	...
	std::thread ted = std::thread([&]() {
		...
		ret = input->open(liveUrl.c_str(), bVideo, bAudio);
		if (ret == 0)
			bOpen = true;
	});
	ted.detach();
...
void FLiveInput::enablePlayAudio(bool bPlay) {
	input->enablePlayAudio(bPlay);
}
...
void FLiveInput::setVideoDataEvent(onVideoFrameHandle onHandle) {
	input->setVideoDataEvent(onHandle);
}

void FLiveInput::setAudioDataEvent(onAudioFrameHandle onHandle) {
	input->setAudioDataEvent(onHandle);
}
