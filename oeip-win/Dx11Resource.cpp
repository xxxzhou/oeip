...
bool Dx11Texture::createResource(ID3D11Device* deviceDx11) {
	...
	if (bCpuWrite) {
		textureDesc.Usage = D3D11_USAGE_DYNAMIC;//DYNAMIC的对应cpu可写，STAGING CPU可读写，否则cpu没任何权限
...
bool Dx11Texture::updateResource(ID3D11DeviceContext* ctxDx11) {
	...
	if ((width % 32) != 0) {
		std::string message = "texture width " + std::to_string(width) + " no mod 32.";
...
bool Dx11Buffer::createResource(ID3D11Device* deviceDx11) {
	...
	//可以在不同的上下文使用
	if (bShared) {
		desc.MiscFlags = D3D11_RESOURCE_MISC_SHARED_KEYEDMUTEX | D3D11_RESOURCE_MISC_BUFFER_STRUCTURED;
	}
	HRESULT result = 0;
	if (cpuData) {
		D3D11_SUBRESOURCE_DATA initData;
	...
	//D3D11 ERROR : ID3D11Device::CreateBuffer : When creating a buffer with the MiscFlag
	//D3D11_RESOURCE_MISC_BUFFER_STRUCTURED specified, the StructureByteStride must be greater than zero, no greater than 2048,
	//and a multiple of 4.[STATE_CREATION ERROR #2097339:
	bBufferInit = SUCCEEDED(result);
	if (bBufferInit && !bNoView) {
...
bool Dx11Buffer::updateResource(ID3D11DeviceContext* ctxDx11) {
	...
	bool bUpdate = updateDx11Resource(ctxDx11, buffer, cpuData, elementByteCount);
	return bUpdate;
}
