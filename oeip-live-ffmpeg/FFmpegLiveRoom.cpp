...
FFmpegLiveRoom::FFmpegLiveRoom() {
	...
	for (int32_t i = 0; i < LIVE_OUTPUT_MAX; i++) {
		...
		liveOuts[i] = std::make_unique< FMuxing>();
	...
	//设定音频输出信息
	onAudioDataHandle dataHandle = std::bind(&FFmpegLiveRoom::onAudioData, this, 0, _1, _2);
...
void FFmpegLiveRoom::onAudioData(int32_t index, uint8_t* data, int32_t size) {
	...
	af.sampleRate = mainPushSetting.audioEncoder.frequency;
	liveOuts[index]->pushAudio(af);
}
...
bool FFmpegLiveRoom::pushStream(int32_t index, const OeipPushSetting& setting) {
	...
	if (index == 0) {
		mainPushSetting = setting;
		if (setting.audioEncoder.bitrate == 0) {
			mainPushSetting.audioEncoder.bitrate = 48000;
		}
		if (setting.audioEncoder.channel == 0) {
			mainPushSetting.audioEncoder.channel = 1;
		}
		if (setting.audioEncoder.frequency == 0) {
			mainPushSetting.audioEncoder.frequency = 32000;
		}
	}
	else if (index == 1) {
		auxPushSetting = setting;
	}
	...
	liveOuts[index]->setAudioEncoder(index == 0 ? mainPushSetting.audioEncoder : auxPushSetting.audioEncoder);
	liveOuts[index]->setVideoEncoder(setting.videoEncoder);
	...
	//}
	auto result = liveOuts[index]->open(uri.c_str(), setting.bVideo, (index == 0 || setting.bAudio));
	//限定只有主流推音频 音频也让外面自己推,主流默认推麦
	if (result >= 0 && index == 0 && !setting.bAudio) {
		audioDesc.bitSize = 16;
		audioDesc.channel = mainPushSetting.audioEncoder.channel;
		audioDesc.sampleRate = mainPushSetting.audioEncoder.frequency;
...
bool FFmpegLiveRoom::pullStream(int32_t userId, int32_t index) {
	...
	if (inIndex < 0) {
		...
		onVideoFrameHandle videoHandle = std::bind(&FFmpegLiveRoom::onVideoFrame, this, userId, index, _1);
