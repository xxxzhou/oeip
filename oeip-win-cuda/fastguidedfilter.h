...
inline __global__ void mainAlign(PtrStepSz<uint16_t> source, PtrStepSz<uint16_t> dest, Intrinsics alignParam) {
	...
	if (idx < source.cols  && idy < source.rows) {
		...
		if (other_xy0.x >= 0 && other_xy0.y >= 0 && other_xy1.x < dest.cols && other_xy1.y < dest.rows) {
...
//		int xmin = min(toperate.left, 1.f - toperate.right)*source.cols;
//		int xmax = max(toperate.left, 1.f - toperate.right)*source.cols;
//		int ymin = min(toperate.top, 1.f - toperate.bottom)*source.rows;
//		int ymax = max(toperate.top, 1.f - toperate.bottom)*source.rows;
...
inline __global__ void distortion(PtrStepSz<uchar4> source, PtrStepSz<uchar4> dest, PtrStepSz<float2> map) {
	...
	if (idx < source.cols && idy < source.rows) {
		...
		float src_x = mapuv.x*source.cols;
		float src_y = mapuv.y*source.rows;
		const int x1 = clamp(__float2int_rd(src_x), 0, source.cols - 1);
		const int y1 = clamp(__float2int_rd(src_y), 0, source.rows - 1);
		const int x2 = x1 + 1;
		const int y2 = y1 + 1;
		const int x2_read = clamp(x2, 0, source.cols - 1);
		const int y2_read = clamp(y2, 0, source.rows - 1);
