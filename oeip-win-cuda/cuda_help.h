...
inline __device__ float __saturatef(float x) {
...
inline __host__ __device__ float3 extractColor(float3 color, float lumaMask) {
	...
	float colorMask = exp(-luma / lumaMask);//exp(-~)=0,exp(0)=1
...
inline __device__ float3 yuv2Rgb(float3 yuv) {
	...
	rgb.y = clamp(yuv.x - 0.71414f * (yuv.z - 0.5f) - 0.34414f * (yuv.y - 0.5f), 0.f, 1.f);
